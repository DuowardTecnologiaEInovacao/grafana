...
sudo apt-get install -y apt-transport-https software-properties-common wget gpg postgresql curl openssl
...
if [ ! -f "$INSTALL_DIR/conf/custom.ini.template" ]; then
    warn "Template não encontrado no pacote. Baixando do GitHub..."
    wget -q -O /tmp/custom.ini.template "$TEMPLATE_URL"
    TEMPLATE_FILE="/tmp/custom.ini.template"
fi

# ===== CORREÇÃO APLICADA AQUI: Trocado o separador de '/' para '@' =====
sudo sed -e "s@%%DB_NAME%%@$DB_NAME@" \
    -e "s@%%DB_USER%%@$DB_USER@" \
    -e "s@%%DB_PASSWORD%%@$DB_PASSWORD@" \
    "$TEMPLATE_FILE" | sudo tee "$CONFIG_FILE" > /dev/null
# ========================================================================
if [ "$TEMPLATE_FILE" == "/tmp/custom.ini.template" ]; then rm /tmp/custom.ini.template; fi
